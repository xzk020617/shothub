...
- [x] 第 2 步：剪贴板闭环——截图自动捕获入库、双击/按钮复制出去（CF_DIB + PNG 双格式、自我写入过滤）
- [ ] 第 3 步：拖拽双向（拖入 / 拖出为文件）
- [ ] 第 4 步：托盘常驻 + 单实例 + 崩溃残留清理
...
.venv\Scripts\python tests\smoke_step2.py   # 剪贴板闭环（14 项）
```
...
app/
  ...
  clipboard_hub.py    # 剪贴板监听 + 自我过滤 + 双格式写回
  widgets.py          # FlowLayout / ThumbnailCard / EmptyState
  mainwindow.py       # 主窗口
tests/                # 离屏冒烟测试
```
